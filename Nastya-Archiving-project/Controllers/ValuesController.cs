...
namespace ElectionsPillars.Controllers
{
    ...
    public class ValuesController : ControllerBase
    {
        ...
        /// <summary>
        /// Fixes common issues with Arabic punctuation and numbers
        /// </summary>
        private string FixArabicPunctuation(string text)
        {
            ...
            try
            {
                // Fix common punctuation spacing issues (remove spaces before punctuation)
                text = text.Replace(" :", ":");
                text = text.Replace(" .", ".");
                text = text.Replace(" ?", "?");
                text = text.Replace(" ?", "?");
                ...
                // Ensure spaces after punctuation for readability
                text = text.Replace(".", ". ");
                text = text.Replace("?", "? ");
                text = text.Replace(":", ": ");
                text = text.Replace("?", "? ");
                ...
                // Fix specific Arabic punctuation issues
                text = text.Replace("??", "?");
                text = text.Replace("..", ".");
                
                // Remove double spaces (loop to handle multiple consecutive spaces)
                while (text.Contains("  "))
                {
                    text = text.Replace("  ", " ");
                }
            ...
            {
                _logger.LogError(ex, "Error in FixArabicPunctuation: {ErrorMessage}", ex.Message);
        ...
        /// <summary>
        /// Determines if the provided text contains Arabic characters
        /// </summary>
        private bool ContainsArabicText(string text, double threshold = 0.1)
        {
            if (string.IsNullOrEmpty(text))
                return false;
                
            try {
                // The Unicode ranges for Arabic characters
                ...
                {
                    // Skip whitespace and common punctuation when counting total chars
                    if (char.IsWhiteSpace(c) || char.IsPunctuation(c) || char.IsDigit(c) || c == '?')
                        continue;
                        
                    totalCharCount++;
                    
                    // Check if character is in any of the Arabic Unicode ranges
                    if ((c >= 0x0600 && c <= 0x06FF) ||   // Arabic Basic
                ...
                // If no meaningful characters found, return false
                if (totalCharCount == 0)
                    return false;
                    
                // Calculate percentage of Arabic characters
                double arabicRatio = (double)arabicCharCount / totalCharCount;
                
                _logger.LogDebug("Arabic detection: {ArabicCount}/{TotalCount} = {Ratio:P2} (Threshold: {Threshold:P2})", 
                    arabicCharCount, totalCharCount, arabicRatio, threshold);
                    
                // Return true if the ratio exceeds the threshold
                return arabicRatio >= threshold;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error detecting Arabic text: {ErrorMessage}", ex.Message);
                // If there's an error in detection, assume it's not Arabic
                return false;
            }
        }
        ...
        private string NormalizeArabicText(string text)
        {
            ...
            try
            {
                // Replace various forms of Alef with standard Alef
                text = text.Replace('?', '?')
                           .Replace('?', '?')
                           .Replace('?', '?');
                
                // Replace various forms of Ya with standard Ya
                text = text.Replace('?', '?')
                           .Replace('?', '?');
                
                // Replace various forms of Waw
                text = text.Replace('?', '?');
                ...
                // Remove any questionable characters that might cause dictionary key issues
                text = text.Replace('?', ' ');
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in NormalizeArabicText: {ErrorMessage}", ex.Message);
                // If normalization fails, return the original text to avoid crashing
                return text;
            }
